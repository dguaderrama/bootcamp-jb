...
> notes: wrangling tabular data (CSV) - naming conventions- no spaces in header!

> wrangling census data - remove all but GEO.id2 and HomeValue/Units - for the purposes of this exercise, we don't need the additional data

> wrangling census data - csvt - HomeValue/Units must be Real numbers for a correct Field Calculation (not sure why yet..)

> wrangling CSVT - to output correct number-types using Field Calculator, use Real(12.0) for HomeValues/Units

> to join multiple tabular data: add shapefile, add CSVs, go to shapefile properties > joins > add joins > SAVE AS new shapefile (these joins are stored in virtual memory, you will return NULL on all records while using field calculator).

> field calc: SEE JOIN MULTIPLE TABULAR DATA. open attributes > edit mode > Field Calculator > enter inputs (reminder: integer * integer = number, integer * string = NULL)
