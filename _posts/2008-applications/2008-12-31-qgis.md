...
Quantify total homve value at risk within landslide/mass-wasting areas.
...
1. Import Census Tracts shapefile
1. View the attribute data
1. Open CSV in text editor or Excel.
 * We're looking for a unique key to join the data.
 * HINT: It's GEOID (tract) and GEO.id2 (ACS)
1. Save housing units CSV as housing-units.csv
1. Save home values CSV as home-values.csv
1. Just wait right there, one does not simply join a string (tract's GEOID) to a number (ACS's GEO.ids).
 * insert an image here of viewing field types in QGIS
1. You'll first have to create a .csvt. Text files do not allow you to specify field types. Having a .csvt in the same directory as your .csv will allow QGIS to read the field type.<br>Copy the line below and paste it into a text editor and save it as housing-units.csv<br>"String","String","String","Integer","Integer"<br>. Create another .csvt with the same text and save it as home-value.csvt. You need to be sure you have defined all fields, e.g. Your .csv has five fields, you must make the .csvt with five fields.
  1. Did you notice anything peculiar about the CSV headers? You're in luck, we did. The headers for the housing attributes are the same in both home-value.csv and housing-units.csv. These fields will overwrite each other if we don't change them now. Look at your metadata to decrypt the headers. Rename the following:<br>home-value.csv:<br>HD01_VD01 = Median Home Value (dollars)<br>HD02_VD01 = Median Home value (MOE)<br>housing-unit.csv:<br>HD01_VD01 = Total Homes<br>HD02_VD01 = Total Homes (MOE)
1. Remove the CSV from your QGIS Layers List
1. Open your revised CSVs in QGIS and confirm that all fields are correct. Notice the fields we need to perform calculations on are integers.
1. Open the Processing Toolbox and search "join attribute table". Open the tool.
1. You must join the CSV (tabular) to the shapefile. You cannot join the shapefile to the tabular data... Configure your inputs as follows:<br>
image here of inputs<br>
notice the Unmatching CRS's. Tabular data is not spatial data, else it's spatial data... Click YES
1. Your output layer is named 'Output Layer', hover over and you can see the file name. Rename this layer to tract-houses.
1. Join home-values.csv to tract-houses.
1. View output-layer attributes. Rename to tract-houses-values. Remove tract-houses, tl_2014_53_tract, home-values, and housing-units. We now have all of our data in one shapefile. And we're still able to do calculations with our interger field (emphasis on the .csvt) 
