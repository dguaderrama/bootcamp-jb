 ...
 * within soft geologic areas 
 ...
 1. Import data:
   ...
   * <em>Add Vector Layer </em>![Spatial Data Bootcamp: Vector Analysis - add vector data]({{site.baseurl}}{{ASSET_PATH}}/images/vector-analysis/vector-analysis-2.png)<br>**fault_100k.shp**<br>
 ...
 1. Buffer fault lines to 100m:
   ...
   * Configure inputs as follows:<br>**Input vector layer: fault_100k<br>Buffer distance: 100<br>Output shapefile: fault_buffer_100m.shp<br>Add results to canvas: (Checked)**<br>
   ...
   * <em>Open the meaure tool</em> ![Spatial Data Bootcamp: Vector Analysis - measure tool]({{site.baseurl}}{{ASSET_PATH}}/images/vector-analysis/measure-tool.png)
   * <em>Measure the width of the buffer.</em> Notice how the total width is only about 62 meters, or 100 feet. Current map units are determined from the CRS, in this case EPSG:2927 is in US feet and we need meters. Simply changing the CRS in Project Properties does not give us the correct results- this is still measured from EPSG:2927 US feet.<br>
   ![Spatial Data Bootcamp: Vector Analysis - measure buffer]({{site.baseurl}}{{ASSET_PATH}}/images/vector-analysis/measure-tool-2.png)<br><br>**Important:**<br>There are two methods to perfoming a buffer analysis on our shapefile. The problem here is that our CRS is EPSG:2927 (in US feet) and the buffer distance is based on map units (still US feet). The two methods are: convert 100 meters to US feet and use this value in the buffer analysis while maintaining EPSG:2927 (in US feet); OR, reproject the layer thus creating a new file with a CRS in meters (correct map units).  <br><br>
   * <em>Open a new project</em> and close without saving current project. This will clear the CRS to avoid confusion.
   * <em>Reopen fault_100k.shp</em> 
   * Reprojecting from EPSG:2927 to EPSG:26910 will change units from feet to meters, allowing to perform a correct buffer analysis (in meters).
   * <em>Reproject fault_100k.shp to ESPG:26910:</em><br>**Reproject from EPSG:2927 (NAD83(HARN) Washington South US feet) to EPSG:26910 (NAD83 UTM10N US meters) - both have same datum (NAD83)**<br>Easiest method is <em>Right-click on layer in layer list > Save As</em> and configure inputs as follows:<br>**Format: ESRI Shapefile<br>Save As: fault_100k_26910.shp<br>CRS: Selected CRS<br>CRS: EPSG: 26910**<br>
   ![Spatial Data Bootcamp: Vector Analysis - wrangling projections]({{site.baseurl}}{{ASSET_PATH}}/images/vector-analysis/vector-analysis-buffer-crs-error-2.png)

   * Configure inputs as follows:<br>**Input vector layer: fault_100k<br>Buffer distance: 100<br>Add results to canvas: (Checked)<br>Output shapefile: faults_100m.shp**<br>
   ![Spatial Data Bootcamp: Vector Analysis - buffer faults]({{site.baseurl}}{{ASSET_PATH}}/images/vector-analysis/vector-analysis-5.png)
